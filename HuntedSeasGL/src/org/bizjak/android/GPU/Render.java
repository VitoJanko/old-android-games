...
public class Render implements Renderer {
	...
	public void onDrawFrame(GL10 arg0) {
		...
		fps = (System.currentTimeMillis() - lastTime) * 0.8 + lastFps * 0.2;
		lastTime = System.currentTimeMillis();
		lastFps = fps;
		
		HUD.setFPS((int) (1000/fps));
		
		Log.d("Render","Around 74.  FPS: "+ 1000/fps);
		...
		if(host.bg != null)
			host.bg.draw(host.renderer.programHandleRipple);
		
		GLES20.glUseProgram(host.renderer.programHandle); // change shader for regular rendering
		for(int i=0; i<host.instances.size(); i++){ 
			try{
				host.instances.get(i).draw();
			}catch(NullPointerException e){
				Log.e("Render","Around 82: "+e.getMessage());
			}
		}
		if(host.fg != null)
			host.fg.draw(host.renderer.programHandleRipple);
	}
    ...
    void makeObject(BufferedReader in) throws IOException{
    	...
    	yEnd = Float.MIN_VALUE;
    	xEnd= Float.MIN_VALUE;
    ...
    private void processVLine(String line){
	    ...
	    for (int i = 1; i < c; i++){
	    	if (i==1){
	    		if (Float.valueOf(tokens[i])>xEnd)
	    			xEnd = Float.valueOf(tokens[i]);
	    		if (Float.valueOf(tokens[i])<xStart)
	    			xStart = Float.valueOf(tokens[i]);
	    	}
	    	if (i==2){
	    		if (Float.valueOf(tokens[i])>yEnd)
	    			yEnd = Float.valueOf(tokens[i]);
	    		if (Float.valueOf(tokens[i])<yStart)
	    			yStart = Float.valueOf(tokens[i]);
	    	}
	...
	public int loadTexture(final Context context, final int resourceId)
    {
        ...
        if (textureHandle[0] != 0)
        {
            ...
            GLUtils.texImage2D(GLES20.GL_TEXTURE_2D, 0, bitmap, 0);//TODO kompresiraj texturo
            
            //Predelan zgornji klic, tako da lahko uporabimo kompresirane texture -> hitrejši load time -> proffit.
//            ByteBuffer buffer = ByteBuffer.allocate(bitmap.getRowBytes()*bitmap.getHeight());
//            Log.d("Render","Tuakj 7");
//            bitmap.copyPixelsToBuffer(buffer);
//            Log.d("Render","Tuakj 8");
...
//            buffer.clear();
//            buffer = null;
//            GLES20.glCompressedTexImage2D(GLES20.GL_TEXTURE_2D, level, internalformat, width, height, border, imageSize, data) //TODO uporabi to
